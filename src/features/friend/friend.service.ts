...
export class FriendService {
	...
	*/
    ...
    if (userId === friendId) {
        throw new HttpException('자기 자신과는 친구가 될 수 없습니다.', HttpStatus.BAD_REQUEST);
    }

    const friendship = await this.friendRepository
			.createQueryBuilder('friend')
			.where('((friend.userId = :userId AND friend.friendId = :friendId) OR (friend.userId = :friendId AND friend.friendId = :userId))', { userId, friendId })
			.getOne();

    if (friendship) {
			if (friendship.status === FriendStatus.Friend) {
				throw new HttpException('이미 친구인 사용자입니다.', HttpStatus.BAD_REQUEST);
	 ...
	 * 친구 삭제
	 */
	async deleteFriend(friendDto: FriendDto): Promise<{ result, message: string }> {
		const { userId, friendId } = friendDto;
		...
		if (userId == friendId) {
			throw new HttpException('You cannot be friends with yourself', HttpStatus.BAD_REQUEST)
		}
		...
		if (friendship) {
			await this.friendRepository.delete({ userId: friendship.userId, friendId: friendship.friendId });
			let message = '';
			if (friendship.status === FriendStatus.Friend) {
				message = '친구 삭제가 완료되었습니다.';
			} else {
				message = friendship.userId === userId ? '친구 요청이 취소되었습니다.' : '친구 요청을 거절하였습니다';
			}
